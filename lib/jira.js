...
export function jiraAuth(creds) {
  const email = creds?.jiraEmail;
  const token = creds?.jiraToken;
  if (!email || !token) throw new Error('Credenciales Jira no configuradas');
  return Buffer.from(`${email}:${token}`).toString('base64');
}

function jiraBase(creds) {
  return creds?.jiraUrl || JIRA_URL;
}

export async function jiraPost(path, body, creds) {
  const auth = jiraAuth(creds);
  const r = await fetch(`${jiraBase(creds)}${path}`, {
    method: 'POST',
...
export async function resolveDisplayNames(accountIds, creds) {
  if (!accountIds?.length) return {};
  const auth = jiraAuth(creds);
  const params = accountIds.map(id => `accountId=${encodeURIComponent(id)}`).join('&');
  const r = await fetch(`${jiraBase(creds)}/rest/api/3/user/bulk?${params}&maxResults=200`, {
...
export async function jiraGet(path, creds) {
  const auth = jiraAuth(creds);
  const r = await fetch(`${jiraBase(creds)}${path}`, {
    headers: { Authorization: `Basic ${auth}`, Accept: 'application/json' },
  });
